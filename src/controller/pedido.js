...
exports.Pedido = {
    ...
    async criarPedido(req, res) {
        const pedido = `INSERT INTO pedido (cod_entidade, valor_total, desconto, valor_liquido, troco, cod_parametro_forma_pagamento, retirada_local, observacao, situacao) values ('${ req.body.cod_entidade }', '${ req.body.valor_total }', '${ req.body.desconto }', '${ req.body.valor_liquido }', '${ req.body.troco }', '${ req.body.cod_parametro_forma_pagamento }', '${ req.body.retirada_local }', '${ req.body.observacao }', '${ req.body.situacao }'); SELECT currval(pg_get_serial_sequence('pedido','cod_pedido'));`
    ...
    async pedidos(req, res) {
        const pedido = `SELECT ped.cod_pedido, cli.nome AS cliente, ped.valor_total, TO_CHAR(ped.data_criacao, 'DD/MM/YYYY HH24:MI:SS') AS data_criacao, pfp.descricao AS forma_pagamento, CASE ped.situacao WHEN 0 THEN 'Pendente' WHEN 1 THEN 'Em andamento' WHEN 2 THEN 'Concluido' END AS situacao, ped.observacao FROM pedido ped INNER JOIN parametro_forma_pagamento pfp ON pfp.cod_parametro_forma_pagamento = ped.cod_parametro_forma_pagamento INNER JOIN entidade cli ON cli.cod_entidade = ped.cod_entidade WHERE ped.situacao ='${ req.params.cod }' AND (ped.data_criacao >='${ req.query.dataIni }' AND ped.data_criacao <='${ req.query.dataFim }') ORDER BY ped.cod_pedido DESC`
        const ped = await db.query(pedido);
    ...
    async pedidosEntidade(req, res) {
        const pedido = `SELECT ped.cod_pedido, ped.valor_total, TO_CHAR(ped.data_criacao, 'DD/MM/YYYY') AS data_criacao, pfp.descricao AS forma_pagamento, CASE ped.situacao WHEN 0 THEN 'Pendente' WHEN 1 THEN 'Em andamento' WHEN 2 THEN 'Concluido' END AS situacao FROM pedido ped INNER JOIN parametro_forma_pagamento pfp ON pfp.cod_parametro_forma_pagamento = ped.cod_parametro_forma_pagamento WHERE cod_entidade='${ req.params.cod }' ORDER BY ped.cod_pedido DESC`
        const ped = await db.query(pedido);
